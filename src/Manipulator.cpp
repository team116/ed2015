...
void Manipulator::process()
{
	...
	if (flap_state == CLOSING && ( (flaps_closed_limit->Get() && using_limits) || arm_timer->HasPeriodPassed(1.1)) ){	//TODO: get real numbers
		close_flaps->Set(0.0);
		flap_state = STILL;
	}
	else if (flap_state == OPENING && ( (flaps_opened_limit->Get() && using_limits) || arm_timer->HasPeriodPassed(1.1)) ){ 	//TODO: get actual numbers
		close_flaps->Set(0.0);
		flap_state = STILL;
	}
	...
	{
		if(current_height < target_height && (!lift_upper_limit->Get() && using_limits)){		//desired height is above current height and upper limit has not been reached
			lifter_one ->Set(0.5);
			lifter_two ->Set(0.5);
		}

		else if(current_height > target_height && (!lift_lower_limit->Get() && using_limits)){		//desired height is below current height
			lifter_one -> Set(-0.5);
			lifter_two -> Set(-0.5);
		}
	}
	...
	if (rake_direction == 1 && ( (port_rake_limit->Get() && using_limits) || rake_timer->HasPeriodPassed(1.1)) ){ 	//TODO: get real times
		rake_port->Set(0.0);
		rake_starboard->Set(0.0);
		rake_direction = 0;
	}
	else if (rake_direction == -1 && rake_timer->HasPeriodPassed(1.1)){ 	//TODO: get real timeout period
		rake_port->Set(0.0);
		rake_starboard->Set(0.0);
		rake_direction = 0;
	}
...
void Manipulator::liftLifters(manual_lifter_controls lifter_direction)
{
	if (lifter_direction == MOVING_UP && (!lift_upper_limit->Get() && using_limits) ){
		lifter_one ->Set(0.5);
		lifter_two->Set(0.5);

	}

	else if (lifter_direction == MOVING_DOWN && (!lift_lower_limit->Get() && using_limits) ){
		lifter_one->Set(-0.5);
		lifter_two->Set(-0.5);

	}
	else if (lifter_direction == NOT_MOVING){
		lifter_one->Set(0.0);
		lifter_two->Set(0.0);
	}
}
